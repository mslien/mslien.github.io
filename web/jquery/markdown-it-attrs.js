...
function bracketed_spans_plugin(md) {
  function span(state, silent) {
    ...
    const labelEnd = md.helpers.parseLinkLabel(state, start, true);
    ...
    const attrEnd = state.src.indexOf('}', attrStart);
    ...
    if (!silent) {
      ...
      const attrs = state.src.slice(attrStart, attrEnd);

      // Create opening span token
      ...
      attrs.split(';').forEach(attr => {
        ...
        if (key && value) {
          tokenOpen.attrs.push([key, value.replace(/^"|"$/g, '')]);
        }
      });
...
// Export the plugin to use it in other files
if (typeof module !== 'undefined') {
  module.exports = bracketed_spans_plugin;
}